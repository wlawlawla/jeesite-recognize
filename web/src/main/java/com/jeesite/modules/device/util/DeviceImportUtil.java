package com.jeesite.modules.device.util;
            ...
            try {
                    ...
                    if (StringUtils.isNotBlank(value)){
                        switch (value){
                            case hard_row_num:{
                                ...
                                cell = row.getCell(1);
                                cell.setCellType(CellType.STRING);
                                currentScreen.setRowNum(Integer.valueOf(cell.getStringCellValue()));
                                cell = row.getCell(3);
                                cell.setCellType(CellType.STRING);
                                currentScreen.setColNum(Integer.valueOf(cell.getStringCellValue()));
                                cell = row.getCell(5);
                                cell.setCellType(CellType.STRING);
                                currentScreen.setStrapNum(Integer.valueOf(cell.getStringCellValue()));
                            }
                            break;
                ...
                if (CollectionUtils.isNotEmpty(deviceInfoVO.getStrapScreenVOList())){
                    deviceInfoVO.getStrapScreenVOList().forEach(strapScreenVO -> {
                        ...
                        if (strapScreenVO.getStrapNum() == null){
                            if (CollectionUtils.isNotEmpty(strapScreenVO.getStrapDetailVOList())){
                                strapScreenVO.setStrapNum(strapScreenVO.getStrapDetailVOList().size());
                            }
                        }
                    });
                }
